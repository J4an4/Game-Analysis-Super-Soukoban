...
void Player::MoveX()
{
	...
	if (state == State::WALKING and look == Look::LEFT)
	{
		...
		if (map->TestBoxLeft(box))
		{
			...
			if (map->MoveBoxLeft(box))
			{
				...
				Tile OldTile = map_two->GetOldTileIndex(x1, pos.y / TILE_SIZE);
				map->ChangeTile(x1, pos.y / TILE_SIZE, OldTile);

				Destination = map_two->GetOldTileIndex(x2, pos.y / TILE_SIZE);
				if (Destination == Tile::FLOORRED)
				{
					map->ChangeTile(x1, pos.y / TILE_SIZE, OldTile);
					if (map->CheckWin(WINCOUNT_LVL1) && level == 1)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL2) && level == 2)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL3) && level == 3)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL4) && level == 4)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL5) && level == 5)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL6) && level == 6)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL7) && level == 7)
					{
						won = true;
					}
					if (map->CheckWin(WINCOUNT_LVL8) && level == 8)
					{
						won = true;
					}
				}
	...
	if (state == State::PUSHING and look == Look::LEFT)
	{
		...
		if (maxMove == pos.x)
		{
			...
			if (Destination == Tile::FLOORRED)
			{
				map->ChangeTile(x2, pos.y / TILE_SIZE, Tile::BOX_STATIC);
				if (map->CheckWin(WINCOUNT_LVL1) && level == 1)
				{
					won = true;
				}
...
void Player::MoveY()
{
	...
	if (state == State::WALKING and look == Look::UP)
	{
		...
		if (map->TestBoxUp(box))
		{
			...
			if (map->MoveBoxUp(box))
			{
				...
				Tile OldTile = map_two->GetOldTileIndex(pos.x / TILE_SIZE, y1);
				map->ChangeTile(pos.x / TILE_SIZE, y1, OldTile);

				Destination = map_two->GetOldTileIndex(pos.x / TILE_SIZE, y2);
				if (Destination == Tile::FLOORRED)
				{
					map->ChangeTile(pos.x / TILE_SIZE, y1, OldTile);
					if (map->CheckWin(WINCOUNT_LVL1) && level == 1)
	...
	if (state == State::PUSHING and look == Look::UP)
	{
		...
		if (maxMove == pos.y)
		{
			...
			if (Destination == Tile::FLOORRED)
			{
				map->ChangeTile(pos.x / TILE_SIZE, y2, Tile::BOX_STATIC);
				if (map->CheckWin(WINCOUNT_LVL1) && level == 1)
				{
					won = true;
				}
